...
class Scope {
  ...
  resolve(name, types) {
    ...
    let refName = `${inflector.camelize(inflector.underscore(name), false)}${this.typesString(types)}`;
  ...
  typesString(types) {
    let string = 'Ref';
    for (let [sigil, type] of [
      ['L', TypeResolution.Local],
      ['C', TypeResolution.Component],
      ['H', TypeResolution.Helper],
      ['P', TypeResolution.Property]
    ]) {
      if (types & type) {
        string += sigil;
      }
...
module.exports = class RootScope extends Scope {
  ...
  // recordYield(index, type) {
  //   // TODO this isn't actually correct; needs to account for inconsistent yields
